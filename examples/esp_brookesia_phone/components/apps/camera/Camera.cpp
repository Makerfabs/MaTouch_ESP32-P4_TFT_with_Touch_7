...
void Camera::onScreenCameraShotBtnClick(lv_event_t *e)
{
    ...
    static uint8_t screenshot_num = 0;
    if(screenshot_num >= 255) screenshot_num = 0;
    char filename[64];
    snprintf(filename, sizeof(filename), BSP_SD_MOUNT_POINT "/screenshot/screenshot_%d.jpg", screenshot_num++);
