...
extern "C" void app_main(void)
{
    ...
    if (reset_reason == ESP_RST_SW || reset_reason == ESP_RST_PANIC) {
        ESP_LOGI(TAG, "Software or panic reset detected, adding delay for hardware stabilization");
    ...
    assert(record != nullptr && "Failed to create record");
    assert((phone->installApp(record) >= 0) && "Failed to begin record");
#endif

    // usb test
    UsbTest *usb_test = new UsbTest();
    assert(usb_test != nullptr && "Failed to create usb_test");
    assert((phone->installApp(usb_test) >= 0) && "Failed to begin usb_test");

    // lcd test
    LcdTest *lcd_test = new LcdTest();
    assert(lcd_test != nullptr && "Failed to create lcd_test");
    assert((phone->installApp(lcd_test) >= 0) && "Failed to begin lcd_test");
